...
// --- CÁC SUB-COMPONENT GIỮ NGUYÊN LOGIC CỦA BẠN ---
...
const KeyCap: React.FC<{ index: number }> = ({ index }) => {
    return (
        <motion.div 
            className={`rounded-[1px] bg-[#1a1a1a] h-2.5`}
            style={{ gridColumn: index === 35 ? 'span 4' : 'span 1' }}
            animate={{ backgroundColor: ["#1a1a1a", "#333333", "#1a1a1a"], scale: [1, 0.95, 1] }}
            transition={{ duration: 0.2, delay: Math.random() * 2, repeat: Infinity, repeatDelay: Math.random() * 3 }}
        />
    )
}

// Visual 3: Smart Automation
    ...
    return (
        <div className="relative w-[300px] h-[300px] flex items-center justify-center">
            ...
            <div className="relative z-10 flex flex-col items-center">
                <WorkflowNode icon={<User className="w-3.5 h-3.5 text-white" />} label="Lead" color="bg-blue-500" delay={0} />
                <ConnectingLine h={25} delay={0.5} />
                <WorkflowNode icon={<Clock className="w-3.5 h-3.5 text-brand-orange" />} label="Wait 2m" color="bg-[#222]" borderColor="border-brand-orange/50" delay={1} />
                <div className="relative h-6 w-px bg-[#333] my-0.5 overflow-hidden">
                     <motion.div className="absolute top-0 left-0 w-full bg-brand-orange" initial={{ height: 0 }} whileInView={{ height: "100%" }} transition={{ duration: 0.5, delay: 1.5 }} />
                </div>
                 <div className="relative w-28 h-px bg-[#333] mb-3">
                     <motion.div className="absolute top-0 left-1/2 w-0 h-full bg-brand-orange" initial={{ width: 0, x: "-50%" }} whileInView={{ width: "100%" }} transition={{ duration: 0.5, delay: 1.8 }} />
                      <motion.div className="absolute left-0 top-0 w-px h-3 bg-brand-orange origin-top" initial={{ scaleY: 0 }} whileInView={{ scaleY: 1 }} transition={{ duration: 0.3, delay: 2.2 }} />
                      <motion.div className="absolute right-0 top-0 w-px h-3 bg-brand-orange origin-top" initial={{ scaleY: 0 }} whileInView={{ scaleY: 1 }} transition={{ duration: 0.3, delay: 2.2 }} />
                 </div>
                <div className="flex gap-6">
                     <WorkflowNode icon={<Mail className="w-3.5 h-3.5 text-purple-400" />} label="Email" color="bg-[#151515]" delay={2.4} />
                     <WorkflowNode icon={<Zap className="w-3.5 h-3.5 text-yellow-400" />} label="CRM" color="bg-[#151515]" delay={2.6} />
                </div>
                 <motion.div initial={{ scale: 0, opacity: 0 }} whileInView={{ scale: 1, opacity: 1 }} transition={{ delay: 3, type: "spring" }} className="absolute -right-4 bottom-2 bg-green-500 text-white text-[9px] font-bold px-2 py-1 rounded-full flex items-center gap-1 shadow-lg">
...
const WorkflowNode = ({ icon, label, color, borderColor, delay }: any) => (
    <motion.div initial={{ scale: 0, opacity: 0 }} whileInView={{ scale: 1, opacity: 1 }} transition={{ delay, type: "spring", stiffness: 200, damping: 15 }} className={`flex items-center gap-2 px-2.5 py-1.5 rounded-md border ${borderColor || 'border-white/10'} ${color || 'bg-[#1a1a1a]'} shadow-xl relative z-10 min-w-[90px] justify-center`}>
        {icon}
...
const ConnectingLine = ({ h, delay }: { h: number, delay: number }) => (
    <div className={`relative w-px bg-[#333] my-0.5 overflow-hidden`} style={{ height: h }}>
        <motion.div className="absolute top-0 left-0 w-full bg-brand-orange" initial={{ height: 0 }} whileInView={{ height: "100%" }} transition={{ duration: 0.5, delay: delay }} />
    </div>
)
